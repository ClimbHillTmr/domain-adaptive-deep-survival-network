...
def _person_period(frame: pd.DataFrame, features: list[str], endpoint: str, interval: int, bins: int):
...
def _fit_source(model, train, val, config, device, seed):
    ...
    x_val, y_val = val
    loader = _loader(x_train, y_train, config["batch_size"], seed)
    positive = y_train.sum()
    loss_fn = nn.BCEWithLogitsLoss(
        pos_weight=torch.tensor((len(y_train) - positive) / positive, device=device)
    )
...
def _fit_update(model, source, target, val, config, device, seed, adversarial_weight):
    ...
    source_loader = _loader(x_source, y_source, config["batch_size"], seed)
    target_loader = _loader(x_target, y_target, config["batch_size"], seed)
    positive = y_target.sum()
    loss_fn = nn.BCEWithLogitsLoss(
        pos_weight=torch.tensor((len(y_target) - positive) / positive, device=device)
    )
    ...
    for epoch in range(config["update_epochs"]):
        ...
        for x_target_batch, y_target_batch in target_loader:
            try:
                x_source_batch, y_source_batch = next(source_iter)
            except StopIteration:
                source_iter = iter(source_loader)
                x_source_batch, y_source_batch = next(source_iter)
            x_target_batch, y_target_batch = x_target_batch.to(device), y_target_batch.to(device)
            x_source_batch, y_source_batch = x_source_batch.to(device), y_source_batch.to(device)
            optimizer.zero_grad()
            coeff = min(1.0, (epoch + 1) / config["update_epochs"])
            target_logit, target_domain = model(x_target_batch, coeff if adversarial_weight else None)
            source_logit, source_domain = model(x_source_batch, coeff if adversarial_weight else None)
            loss = loss_fn(target_logit, y_target_batch)
            loss += config["source_replay_weight"] * loss_fn(source_logit, y_source_batch)
            if adversarial_weight:
                domain_logits = torch.cat((source_domain, target_domain))
                domain_labels = torch.cat(
                    (
                        torch.zeros(len(source_domain), dtype=torch.long, device=device),
                        torch.ones(len(target_domain), dtype=torch.long, device=device),
                    )
                )
            ...
            optimizer.step()
        auc = _validation_auc(model, *val, device)
        if auc > best_auc:
            best_state, best_auc, remaining = copy.deepcopy(model.state_dict()), auc, config["patience"]
        else:
            remaining -= 1
...
def main() -> None:
    ...
    for seed in config["training"]["initialization_seeds"]:
        ...
        for endpoint in ENDPOINTS:
            source_train = _person_period(bundle.source_train, bundle.feature_names, endpoint, model_config["interval_minutes"], bins)
            source_val = _person_period(bundle.source_val, bundle.feature_names, endpoint, model_config["interval_minutes"], bins)
            target_train = _person_period(bundle.target_train, bundle.feature_names, endpoint, model_config["interval_minutes"], bins)
            target_val = _person_period(bundle.target_val, bundle.feature_names, endpoint, model_config["interval_minutes"], bins)
