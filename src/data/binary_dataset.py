...
def prepare_binary_data(
    ...
    *,
    ...
    cache_dir = Path(source_path).parent / "cache"
    cache_file = cache_dir / "preprocessed_data.pkl"
    
    if use_cache and cache_file.is_file():
        try:
            with open(cache_file, 'rb') as f:
                cache_data = pickle.load(f)
            return BinaryDataBundle(
                cache_data["source_train"],
                cache_data["source_val"],
                cache_data["target_train"],
                cache_data["target_val"],
                cache_data["target_test"],
                cache_data["feature_names"],
                cache_data["preprocessing"],
                cache_data["split_manifest"],
            )
        except Exception:
            pass
