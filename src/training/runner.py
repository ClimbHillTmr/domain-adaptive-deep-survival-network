...
from src.evaluation.classification import compute_basic_metrics, per_stage_recall, brier_score, stage_misalignment, confusion
from src.calibration.stage_calibration import calibrate_per_stage, optimize_thresholds_fbeta, bootstrap_ci_per_stage, plot_calibration_curves
...
def run_once(X, y_stage, save_dir="./runs", model_type='lgbm', X_seq=None, X_static=None, survival=False, boundaries=(30,90), durations=None, events=None):
    ...
    if model_type == 'lgbm':
        ...
        cw = {int(c): float(total / (len(unique) * n)) for c, n in zip(unique.tolist(), counts.tolist())}
        clf = LightGBMStageClassifier(class_weight=cw)
        clf.fit(X, y_stage)
    ...
    np.savetxt(os.path.join(out, "stage_probabilities.csv"), proba, delimiter=",", fmt="%.6f")
    np.savetxt(os.path.join(out, "stage_prob_ci_lower.csv"), lower, delimiter=",", fmt="%.6f")
    np.savetxt(os.path.join(out, "stage_prob_ci_upper.csv"), upper, delimiter=",", fmt="%.6f")
    ...
    try:
        from src.calibration.stage_calibration import calibrate_per_stage, apply_thresholds_predict, optimize_thresholds_fbeta_per_class
        ...
        with open(os.path.join(out, "thresholds.json"), "w") as f:
            json.dump({"beta": betas, "thresholds": list(map(float, th)), "f_beta": list(map(float, fb))}, f, indent=2)
        from src.calibration.stage_calibration import optimize_thresholds_with_distribution
        target_props = [float(np.mean(y_stage==c)) for c in range(proba_cal.shape[1])]
        th_dist = optimize_thresholds_with_distribution(proba_cal, y_stage, target_props=target_props, beta=2.0, min_thresholds=[0.15,0.08,0.12,0.12])
        preds_th = apply_thresholds_predict(proba_cal, th_dist, priority=[1,0,2,3], weights=[1.12,1.08,1.0,0.98], min_thresholds=[0.15,0.08,0.12,0.12], conf_min=0.30)
        metrics_th = {}
    ...
    if survival and durations is not None and events is not None:
        try:
            ...
            soft_probs = map_survival_to_stage_probs(sf.iloc[:, 0], boundaries=boundaries)
            # HR/CI输出
            exp = cox.explain()
            import json
            with open(os.path.join(out, "coefficients.csv"), "w") as f:
                ...
                for k in exp['coefficients'].keys():
                    f.write(f"{k},{exp['coefficients'][k]},{exp['hazard_ratio'][k]},{exp['ci_lower'][k]},{exp['ci_upper'][k]},{exp['p_values'][k]}\n")
            try:
                import numpy as np
                sp = np.array(soft_probs, dtype=float).reshape(1, -1)
                np.savetxt(os.path.join(out, "stage_soft_vs_pred.csv"), np.concatenate([sp, proba[:1]], axis=1), delimiter=",", fmt="%.6f")
            except Exception:
                pass
        except Exception:
            pass
...
def run_train_external(X_train, y_train, X_ext=None, y_ext=None, save_dir="./runs", model_type='lgbm', boundaries=(30,90),
                       X_seq_train=None, X_static_train=None, X_seq_ext=None, X_static_ext=None,
                       survival=False, durations=None, events=None):
    ...
    if model_type == 'lgbm':
        ...
        if X_ext is not None and y_ext is not None:
            ...
            X_ext_a = _coral_align(X_train_s, X_ext_s)
            ...
            try:
                from src.calibration.stage_calibration import calibrate_per_stage, apply_thresholds_predict, optimize_thresholds_fbeta_per_class, optimize_thresholds_with_distribution
                train_props = [float(np.mean(y_train==c)) for c in range(proba_ext.shape[1])]
                ext_props = [float(np.mean(y_ext==c)) for c in range(proba_ext.shape[1])]
                w = np.array([(train_props[c] + 1e-6) / (ext_props[c] + 1e-6) for c in range(proba_ext.shape[1])], dtype=float)
                w = np.clip(w, 0.8, 1.5)
                ...
                th_ext, fb_ext = optimize_thresholds_fbeta_per_class(proba_ext_cal, y_ext, betas_ext)
                with open(os.path.join(out, "thresholds_external.json"), "w") as f:
                    json.dump({"beta": betas_ext, "thresholds": list(map(float, th_ext)), "f_beta": list(map(float, fb_ext))}, f, indent=2)
                target_props_ext = [float(np.mean(y_ext==c)) for c in range(proba_ext_cal.shape[1])]
                target_props_ext = [max(p, 0.01) for p in target_props_ext]
                th_dist_ext = optimize_thresholds_with_distribution(proba_ext_cal, y_ext, target_props=target_props_ext, beta=2.0, min_thresholds=[0.12,0.10,0.12,0.12])
                preds_ext_th = apply_thresholds_predict(proba_ext_cal, th_dist_ext, priority=[2,1,3,0], weights=[1.05,1.02,1.18,1.12], min_thresholds=[0.12,0.10,0.12,0.12], conf_min=0.30)
                metrics_ext_th = {}
                ...
                with open(os.path.join(out, "external_metrics_thresholded.json"), "w") as f:
                    json.dump(metrics_ext_th, f, indent=2)
                plot_calibration_curves(proba_ext_cal, y_ext, os.path.join(out, "calibration_external"))
            except Exception:
                pass
    else:
        from sklearn.preprocessing import RobustScaler
        ...
        wrapper = TorchStageWrapper(model)
        proba_tr = wrapper.predict_proba(X_seq_train, X_static_train)
        ...
        with open(os.path.join(out, "internal_metrics.json"), "w") as f:
            json.dump(metrics_int, f, indent=2)
        try:
            th_tr, fb_tr = optimize_thresholds_fbeta(proba_tr[:min_len], y_train, beta=2.0)
            with open(os.path.join(out, "thresholds_internal.json"), "w") as f:
                json.dump({"beta": 2.0, "thresholds": th_tr, "f_beta": fb_tr}, f, indent=2)
            plot_calibration_curves(proba_tr[:min_len], y_train, os.path.join(out, "calibration_internal"))
        except Exception:
            pass
        if X_seq_ext is not None and X_static_ext is not None and y_ext is not None:
            ...
            if X_static_train is not None and X_static_ext is not None:
                ...
                X_static_ext_a = _coral_align(X_static_train_s, X_static_ext_s)
            proba_ext = wrapper.predict_proba(X_seq_ext, X_static_ext_a if X_static_ext_a is not None else X_static_ext)
            ...
            with open(os.path.join(out, "external_metrics.json"), "w") as f:
                json.dump(metrics_ext, f, indent=2)
            try:
                from src.calibration.stage_calibration import calibrate_per_stage, optimize_thresholds_fbeta_per_class, optimize_thresholds_with_distribution, apply_thresholds_predict
                train_props = [float(np.mean(y_train==c)) for c in range(proba_ext.shape[1])] if y_train is not None else [0.25]*proba_ext.shape[1]
                ext_props = [float(np.mean(y_ext==c)) for c in range(proba_ext.shape[1])]
                w = np.array([(train_props[c] + 1e-6) / (ext_props[c] + 1e-6) for c in range(proba_ext.shape[1])], dtype=float)
                w = np.clip(w, 0.8, 1.5)
                ...
                th_ext, fb_ext = optimize_thresholds_fbeta_per_class(P_cal, y_ext, betas_ext)
                with open(os.path.join(out, "thresholds_external.json"), "w") as f:
                    json.dump({"beta": betas_ext, "thresholds": list(map(float, th_ext)), "f_beta": list(map(float, fb_ext))}, f, indent=2)
                target_props_ext = [float(np.mean(y_ext==c)) for c in range(P_cal.shape[1])]
                target_props_ext = [max(p, 0.01) for p in target_props_ext]
                th_dist_ext = optimize_thresholds_with_distribution(P_cal, y_ext, target_props=target_props_ext, beta=2.0, min_thresholds=[0.12,0.05,0.12,0.12])
                preds_ext_th = apply_thresholds_predict(P_cal, th_dist_ext, priority=[1,0,2,3], weights=[1.15,1.08,1.0,0.98], min_thresholds=[0.12,0.05,0.12,0.12], conf_min=0.25)
                metrics_ext_th = {}
                metrics_ext_th.update(compute_basic_metrics(y_ext, preds_ext_th))
                metrics_ext_th.update(per_stage_recall(y_ext, preds_ext_th))
                metrics_ext_th.update(stage_misalignment(y_ext, preds_ext_th))
                ...
                with open(os.path.join(out, "external_metrics_thresholded.json"), "w") as f:
                    json.dump(metrics_ext_th, f, indent=2)
                plot_calibration_curves(P_cal, y_ext, os.path.join(out, "calibration_external"))
            except Exception:
                pass
        # 生存辅助（仅当传入durations/events且survival=True）
        if survival and durations is not None and events is not None and X_static_train is not None:
            try:
                import pandas as pd
                ...
                try:
                    sp = np.array(soft_probs, dtype=float).reshape(1, -1)
                    np.savetxt(os.path.join(out, "stage_soft_vs_pred.csv"), np.concatenate([sp, proba_tr[:1]], axis=1), delimiter=",", fmt="%.6f")
                except Exception:
                    pass
            except Exception:
                pass
