...
def predict_mlp(model, x: np.ndarray, device: torch.device, is_cdan: bool = False, batch_size: int = 1024) -> np.ndarray:
    model.eval()
    with torch.no_grad():
...
def _fit_mlp_phase(
    ...
    *,
    ...
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, generator=generator, pin_memory=True)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for batch in loader:
            ...
            if coral_weight > 0.0 and x_target_train is not None:
                ...
                target_features = target_batch[0].to(device, non_blocking=True)
                if x_source_alignment is not None:
                    try:
                        ...
                        source_alignment_iter = iter(source_alignment_loader)
                        source_batch = next(source_alignment_iter)
                    alignment_source_features = source_batch[0].to(device, non_blocking=True)
                else:
        ...
        with torch.no_grad():
            ...
            probs = []
            eval_batch_size = min(2048, n_val)
            for i in range(0, n_val, eval_batch_size):
                batch = x_val_tensor[i:i+eval_batch_size]
                if is_cdan:
                    logits, _, _ = model(batch)
                else:
                    logits = model(batch)
                probs.append(torch.sigmoid(logits).cpu())
            val_probs = torch.cat(probs).numpy()
