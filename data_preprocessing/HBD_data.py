...
def shengyi_dataset(first_pressure_sd="透前动脉压", use_parallel=True, n_threads=30):
    """主函数，支持选择是否使用并行处理"""
    # 检查优化数据文件是否存在
    optimized_file_path = "/home/cht/Works/PredictionTimeHypotensionDialysis/data_preprocessing/data/深医_optimized_data透前动脉压.csv"
...
def process_historical_averages(dataset, first_pressure_sd='', use_parallel=True, n_threads=20):
    ...
    target_file = f'/home/cht/Works/PredictionTimeHypotensionDialysis/data_preprocessing/data/深医_result_df{first_pressure_sd}.csv'
    ...
    if os.path.exists(target_file):
        ...
        if use_parallel:
            ...
            print(result_df)
            result_df.to_csv(
                path_or_buf=("/home/cht/Works/PredictionTimeHypotensionDialysis/data_preprocessing/data/深医_result_df" + str(first_pressure_sd) + ".csv")
            )
    ...
    final_data.to_csv(
        path_or_buf=("/home/cht/Works/PredictionTimeHypotensionDialysis/data_preprocessing/data/深医_final_data.csv")
    )
