...
| 属性     | 内容                                                        |
| -------- | ----------------------------------------------------------- |
| 项目名称 | domain-adaptive-deep-survival-network                       |
| 文档日期 | 2026-05-02                                                  |
| 文档版本 | v5.0 (关键 Bug 修复 + 架构优化)                             |
| 文档状态 | 已更新 - IBS/动态特征/Cross-Attention/配置/异常处理全面修复 |
...
10. [项目组织结构](#10-项目组织结构)
11. [v5.0 关键 Bug 修复记录 (2026-05-02)](#11-v50-关键-bug-修复记录-2026-05-02)
12. [最新进展与待完成实验 (2026-04-25)](#12-最新进展与待完成实验-2026-04-25)
...
## 11. v5.0 关键 Bug 修复记录 (2026-05-02)

### 11.1 修复概览

| # | 问题 | 严重性 | 修复文件 | 修复内容 |
|---|------|--------|----------|----------|
| 1 | **IBS 计算错误**（值 >1.0） | 严重 | `src/metrics/survival_metrics.py` | 使用 Kaplan-Meier 基线估计将 risk scores 转换为生存概率，调用 scikit-survival 的 `brier_score` |
| 2 | **动态特征处理不当** | 严重 | `src/models/dadsn_model.py` | 创建 `DynamicFeatureEncoder`，seq_len≥4 用 ResNet1D，seq_len=1 用 MLP |
| 3 | **Cross-Attention 退化** | 中等 | `src/models/dadsn_model.py` | 创建 `GatedFusion` 模块，在 seq_len=1 时替代 Cross-Attention |
| 4 | **配置项命名混乱** | 低 | `configs/defaults.yaml` | `lambda_coral` → `domain_adaptation.lambda_da`，支持 MMD/CORAL/DANN |
| 5 | **SHAP 分析误称** | 低 | `src/evaluation/shap_analysis.py` | 明确标注为 "Perturbation-based Feature Importance" |
| 6 | **随机种子不完整** | 低 | `src/utils/seed_utils.py` | 新增完整种子设置工具，支持 CUDA 确定性模式 |
| 7 | **宽泛异常处理** | 低 | `survival_metrics.py`, `dadsn_runner.py` | 替换 `except Exception` 为具体异常类型 |
| 8 | **训练日志不清晰** | 低 | `dadsn_runner.py` | 日志输出显示 DA 方法名称，支持向后兼容 |

### 11.2 详细修复说明

#### 修复 1: IBS 计算错误

**问题**：原始实现直接使用 raw risk scores（log-hazard ratios）计算 Brier Score，产生不可能的值（>1.0）。Brier Score 要求输入为 [0,1] 范围内的生存概率。

**根因**：`integrated_brier_score()` 函数未将模型输出的 log-hazard ratio 转换为生存概率。

**修复方案**：
1. 实现 `_risk_to_survival_probability()` 函数，使用 Kaplan-Meier 基线估计
2. 将 risk scores 通过 `S(t|x) = S_0(t)^exp(h(x))` 转换为生存概率
3. 调用 scikit-survival 的 `brier_score` 函数进行正确的 IBS 计算

**影响**：IBS 值现在正确落在 [0,1] 范围内，可用于模型校准评估。

#### 修复 2: 动态特征处理不当

**问题**：`ResNet1DEncoder` 被用于单帧数据（seq_len=1），浪费时序建模能力且效率低下。

**根因**：当前数据仅有汇总统计特征（mean/std），seq_len=1，但模型仍使用 1D 卷积处理。
...
class DynamicFeatureEncoder(nn.Module):
    ...
    def __init__(self, num_features, d_model, seq_len=1, dropout=0.1):
        if seq_len >= 4:
            self.encoder_type = "resnet1d"  # 时序模式
            self.resnet_encoder = ResNet1DEncoder(...)
        else:
            self.encoder_type = "mlp"  # 汇总统计模式
            self.mlp_encoder = nn.Sequential(...)
```

**影响**：
- seq_len=1 时参数量减少 ~60%，训练速度提升
- 未来接入原始时序数据时自动切换到 ResNet1D

#### 修复 3: Cross-Attention 退化

**问题**：当 seq_len=1 时，Cross-Attention 退化为双线性变换，失去注意力机制的意义。

**根因**：Cross-Attention 需要序列维度来计算 query-key 相似度，seq_len=1 时注意力权重恒为 1。

**修复方案**：创建 `GatedFusion` 模块，使用可学习门控控制信息流：
```python
class GatedFusion(nn.Module):
    def forward(self, static_feat, dyn_feat):
        gate_s = sigmoid(Linear(cat([static, dyn])))
        gate_d = sigmoid(Linear(cat([static, dyn])))
        return static * gate_s + static, dyn * gate_d + dyn
```

**影响**：在 seq_len=1 时提供更合理的特征融合机制。
...
**影响**：支持多种域适应方法，命名更清晰。向后兼容旧配置格式。

### 11.3 Git 分支状态

| 分支 | 远程仓库 | 状态 |
|------|----------|------|
| `main` | `PredictionTimeHypotensionDialysis` | 原始代码 |
| `NEW` | `domain-adaptive-deep-survival-network` | 包含所有 v5.0 修复 |

### 11.4 已知问题（待修复）

- [ ] 伪标签自训练逻辑（避免自证预言循环）
- [ ] 临床基线对比实验缺失
- [ ] 动态特征仍为汇总统计模式，未接入原始时序数据

---

## 12. 最新进展与待完成实验 (2026-04-25)
...
#### Experiment 1: Base (Static Only + Cox) - 已完成

| Seed     | Source C-index      | Target C-index      | Early Stop |
| -------- | ------------------- | ------------------- | ---------- |
| 42       | 0.6429              | 0.3445              | Epoch 24   |
| 123      | 0.6347              | 0.3414              | Epoch 14   |
| 456      | 0.6307              | 0.3428              | Epoch 11   |
| **平均** | **0.6361 ± 0.0051** | **0.3429 ± 0.0012** | -          |

**关键发现**：Base 模型在福鼎（目标域）的 C-index ≈ 0.34，**低于随机猜测 (0.5)**，证明：
1. 深医和福鼎的数据分布差异极大
2. 无域适应的模型无法跨中心泛化
3. 预测风险排序与实际完全相反（过拟合源域事件样本）
...
### 11.3 待完成实验（需在 GPU 服务器上运行）

#### 实验目标

验证架构优化（Cross-Attention + 更深网络 + Cosine Annealing）能否将 DA-DSN (MMD) 的 C-index 提升至 **0.65+**。
...
| 实验                       | 配置                         | 预期 C-index |
| -------------------------- | ---------------------------- | ------------ |
| **Exp 1: Base**            | Static Only + Cox            | 0.55-0.56    |
| **Exp 2: Base + ResNet1D** | Static + Dynamic + Cox       | 0.58-0.60    |
| **Exp 3: DA-DSN (MMD)**    | Static + Dynamic + Cox + MMD | **0.65+**    |

#### 运行命令

```bash
# 在 GPU 服务器上执行
cd /path/to/PredictionTimeHypotensionDialysis
...
| 实验                  | CPU (当前)   | GPU (预计)  |
| --------------------- | ------------ | ----------- |
| Exp 1 (Base)          | ~2 小时      | ~15 分钟    |
| Exp 2 (Base+ResNet1D) | ~3 小时      | ~25 分钟    |
| Exp 3 (DA-DSN+MMD)    | ~3 小时      | ~25 分钟    |
| **总计 (3 seeds)**    | **~24 小时** | **~2 小时** |
