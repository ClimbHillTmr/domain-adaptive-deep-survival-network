...
FEATURE_NAMES = [
    ...
    "透析液钙浓度", "透析液电导率", "实际透析时长",
    # Pre-dialysis Status
    "透前体重-干体重", "透前呼吸频率", "透前体温", "透前收缩压", "透前舒张压", "透前动脉压",
    # Ultrafiltration
    "超滤率_绝对", "超滤率_体重归一化", "超滤率高危Flag", "超滤量MAX",
    "脉压差", "平均动脉压"
]
...
def build_feature_tables(source_path: str, target_path: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[str]]:
    """
...
def prepare_dataloaders(
    ...
    x_s, x_t, e_s, t_s, e_t, t_t, feature_names = build_feature_tables(source_path, target_path)
