...
from src.models.dadsn_model import DADSN, cox_partial_log_likelihood, coral_loss, mmd_loss, dann_loss, joint_mmd_loss, joint_coral_mmd_loss, DomainClassifier
...
def train_epoch_dadsn(
    ...
    for batch_static, batch_dyn, batch_event, batch_time in loader_src:
        ...
        if coral_lambda > 0 and iter_tgt:
            ...
            # Select domain adaptation method
            if da_method == "coral":
                da_loss = coral_loss(emb_fuse, emb_tgt)
            elif da_method == "mmd":
                da_loss = mmd_loss(emb_fuse, emb_tgt)
            elif da_method == "joint_mmd":
                # Joint MMD with pseudo-labels
                da_loss = joint_mmd_loss(
                    emb_fuse, emb_tgt,
                    source_labels=log_hazard.squeeze().detach(),
                    target_labels=model(tgt_static, tgt_dyn)[0].squeeze().detach(),
                    conditional_weight=0.3
                )
            elif da_method == "joint_coral_mmd":
                # Joint CORAL + MMD with adaptive weights
                # Early epochs: focus on CORAL (covariance alignment)
                # Later epochs: increase MMD weight (distribution alignment)
                progress = min(1.0, current_epoch / da_warmup_epochs)
                coral_w = 0.7 - 0.2 * progress  # 0.7 → 0.5
                mmd_w = 0.3 + 0.2 * progress    # 0.3 → 0.5
                da_loss = joint_coral_mmd_loss(
                    emb_fuse, emb_tgt,
                ...
                if domain_classifier is not None:
                    ...
                    alpha = dann_alpha * min(1.0, current_epoch / da_warmup_epochs)
                    da_loss = dann_loss(emb_fuse, emb_tgt, domain_classifier, alpha)
                else:
                    da_loss = torch.tensor(0.0, device=device)
            else:
                da_loss = torch.tensor(0.0, device=device)

            loss = loss + coral_lambda * da_loss
    ...
    avg_da = total_da_loss / max(n_batches, 1)
    return avg_surv, avg_da
...
def run_dadsn_experiment(
    ...
    for seed in seeds:
        ...
        # Create domain classifier for DANN
        domain_classifier = None
        dann_alpha = 1.0
        if da_method == "dann" and da_enabled:
            transformer_cfg = (
                config.model.transformer if hasattr(config, "model") else config.transformer
            )
            d_model = transformer_cfg.d_model
            domain_classifier = DomainClassifier(d_model, dropout=0.1).to(device)
            dann_alpha = getattr(da_config, 'alpha', 1.0)
