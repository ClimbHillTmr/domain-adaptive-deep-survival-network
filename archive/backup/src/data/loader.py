...
class DialysisDataLoader:
    ...
    def _process_static(self, df, is_training):
        ...
        for col in missing_cols:
            X_static[col] = 0.0  # Default value for missing features
        
        # Reorder columns to match config
        X_static = X_static[static_cols]
        ...
        # 6. SBP drop during dialysis = 透前收缩压 - 透后收缩压
        # Larger drops indicate hemodynamic instability
        if "透前收缩压" in X_static.columns and "透后收缩压" in X_static.columns:
            X_static["收缩压降幅"] = X_static["透前收缩压"] - X_static["透后收缩压"]

        # 7. DBP drop during dialysis = 透前舒张压 - 透后舒张压
        if "透前舒张压" in X_static.columns and "透后舒张压" in X_static.columns:
            X_static["舒张压降幅"] = X_static["透前舒张压"] - X_static["透后舒张压"]

        # 8. Arterial pressure drop = 透前动脉压 - 透后动脉压
        if "透前动脉压" in X_static.columns and "透后动脉压" in X_static.columns:
            X_static["动脉压降幅"] = X_static["透前动脉压"] - X_static["透后动脉压"]

        # 9. Post-dialysis pulse pressure = 透后收缩压 - 透后舒张压
        if "透后收缩压" in X_static.columns and "透后舒张压" in X_static.columns:
            X_static["透后脉压差"] = X_static["透后收缩压"] - X_static["透后舒张压"]

        # 10. SBP variability normalized by mean (变异系数)
        if "透析中收缩压_std" in X_static.columns and "透析中收缩压_mean" in df.columns:
            mean_sbp = df["透析中收缩压_mean"].replace(0, np.nan)
            X_static["收缩压变异系数"] = X_static["透析中收缩压_std"] / mean_sbp
            X_static["收缩压变异系数"] = X_static["收缩压变异系数"].fillna(0)
