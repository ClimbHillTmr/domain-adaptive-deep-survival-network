"""
v11.3 landmark-60 clean validation + strictly pre-60-min dynamic statistics.
...
2. Landmark task: only sessions still at risk at 60 min are kept.
3. Clean target split: adapt_train / adapt_val / test.
4. Early stopping based on adapt_val only.
"""
...
CONFIG = {
    ...
    "seed": 42,
    "results_path": "runs/v11_3_landmark60_da_results.json",
}
...
def load_feature_matrices():
    ...
    meta = {
        "source_original_n": int(len(pd.read_csv(CONFIG["source_data"]))),
        "target_original_n": int(len(pd.read_csv(CONFIG["target_data"]))),
...
def main():
    ...
    print("1. Loading upstream landmark-60 feature matrices...")
    (
        x_source,
        e_source,
        t_source,
    ...
    print(f"V11.3 landmark DA test    : {da_test:.4f}")
