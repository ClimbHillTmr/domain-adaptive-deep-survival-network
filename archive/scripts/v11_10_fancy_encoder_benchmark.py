"""
v11.10 benchmark fancy encoders on top of v11.6 features and DA pipeline.

Encoders:
1. TabTransformer-style encoder for continuous tabular features
2. KAN-style basis encoder using per-feature radial basis expansions
"""
...
CONFIG.update(
    {
        "results_path": "runs/v11_10_fancy_encoder_benchmark_results.json",
        "seed": 42,
        "d_model": 64,
        "dropout": 0.15,
        "lr": 8e-4,
...
class KANStyleBasisLayer(nn.Module):
    def __init__(self, input_dim, basis_dim):
        super().__init__()
        self.input_dim = input_dim
        self.basis_dim = basis_dim
        centers = torch.linspace(-3.0, 3.0, basis_dim)
        self.register_buffer("centers", centers)
        self.log_width = nn.Parameter(torch.zeros(input_dim, basis_dim))
        self.base_weight = nn.Parameter(torch.ones(input_dim))
        self.basis_weight = nn.Parameter(torch.randn(input_dim, basis_dim) * 0.05)
    ...
    def forward(self, x):
        ...
        spline_term = (basis * self.basis_weight.unsqueeze(0)).sum(dim=-1)
        out = self.base_weight.unsqueeze(0) * x + spline_term + self.bias.unsqueeze(0)
        return out


class KANStyleSurvivalNet(nn.Module):
    def __init__(self, input_dim, d_model, basis_dim, dropout):
        super().__init__()
        self.kan = KANStyleBasisLayer(input_dim, basis_dim)
    ...
    with torch.no_grad():
        for start in range(0, len(x_eval), CONFIG["eval_batch_size"]):
            ...
            x_batch = torch.tensor(
                x_eval[start:end], dtype=torch.float32, device=CONFIG["device"]
            )
...
def set_trainable_state(model, phase, model_name):
    ...
    if model_name == "tabtransformer":
        ...
        if phase == "full_finetune":
            for param in (
                model.feature_weight.parameters()
                if hasattr(model.feature_weight, "parameters")
                else []
            ):
                param.requires_grad = True
...
def build_optimizer(model, phase, model_name):
    if phase == "head_only":
        return optim.AdamW(
    ...
    if model_name == "tabtransformer":
        ...
        if phase == "partial_unfreeze":
            param_groups = [
                {"params": model.hazard_head.parameters(), "lr": CONFIG["head_lr"]},
                {
                    "params": model.encoder[4].parameters(),
                    "lr": CONFIG["top_encoder_lr"],
                },
                {
                    "params": model.encoder[5].parameters(),
                    "lr": CONFIG["top_encoder_lr"],
                },
            ]
            return optim.AdamW(param_groups, weight_decay=1e-3)
        if phase == "full_finetune":
            param_groups = [
                ...
                {"params": model.kan.parameters(), "lr": CONFIG["full_lr"]},
            ]
            return optim.AdamW(param_groups, weight_decay=1e-4)
...
def run_head_only_baseline(model, target_train_loader, x_val, e_val, t_val, model_name):
    set_trainable_state(model, "head_only", model_name)
    optimizer = build_optimizer(model, "head_only", model_name)
...
def run_da_phase(
    ...
    set_trainable_state(model, phase, model_name)
    optimizer = build_optimizer(model, phase, model_name)
    best_val = evaluate_cindex(model, x_val, e_val, t_val)
    best_epoch = 0
    best_state = copy.deepcopy(model.state_dict())
...
def build_model(model_name, input_dim):
    if model_name == "tabtransformer":
        return TabTransformerEncoderNet(
            input_dim=input_dim,
    ...
    if model_name == "kanstyle":
        return KANStyleSurvivalNet(
            input_dim=input_dim,
            d_model=CONFIG["d_model"],
            basis_dim=CONFIG["kan_basis_dim"],
            dropout=CONFIG["dropout"],
        ).to(CONFIG["device"])
...
def run_model(
    ...
    base_model = build_model(model_name, input_dim)
    ...
    head_model = build_model(model_name, input_dim)
    head_model.load_state_dict(source_state)
    head_val, head_epoch, _ = run_head_only_baseline(
        head_model, target_train_loader, x_val, e_val, t_val, model_name
    )
    head_test = evaluate_cindex(head_model, x_test, e_test, t_test)

    da_model = build_model(model_name, input_dim)
...
def main():
    ...
    print("Loading v11.6 feature tables for fancy encoder benchmark...")
    ...
    for model_name in ["tabtransformer", "kanstyle"]:
        print(f"\n=== Running {model_name} ===")
        results["models"][model_name] = run_model(
            model_name,
            x_source,
            e_source,
            t_source,
        ...
        print(
            f"{model_name}: zero-shot={model_result['source_pretrain']['zero_shot_test_cindex']:.4f}, "
            f"da={model_result['layerwise_da']['test_cindex']:.4f}, "
            f"gain={model_result['layerwise_da']['gain_vs_zero_shot']:.4f}"
        )
