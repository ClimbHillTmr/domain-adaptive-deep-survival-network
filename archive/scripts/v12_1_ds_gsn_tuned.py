"""
V12.1: Domain-Stratified Gated Survival Network (DS-GSN) Tuning & Architecture Upgrade
Upgrades:
1. CLS Token Pooling: Replaced Mean Pooling with a learnable [CLS] token for survival prediction to prevent dilution of critical deterioration signals.
2. Mask L1 Regularization: Added a sparsity penalty to the Gated DANN mask to force it to actively drop features causing negative transfer.
3. Local Hyperparameter Sweep: Tuning adv_weight and mask_l1_weight.
"""
...
CONFIG.update(
    {
        "results_path": "runs/v12_1_ds_gsn_tuned_results.json",
        "seed": 42,
...
LOCAL_SWEEPS = [
    {"name": "cls_adv0.05_l1_0.0", "adv_weight": 0.05, "mask_l1_weight": 0.0},
    {"name": "cls_adv0.05_l1_0.01", "adv_weight": 0.05, "mask_l1_weight": 0.01},
    {"name": "cls_adv0.05_l1_0.05", "adv_weight": 0.05, "mask_l1_weight": 0.05},
    {"name": "cls_adv0.10_l1_0.01", "adv_weight": 0.10, "mask_l1_weight": 0.01},
    {"name": "cls_adv0.02_l1_0.01", "adv_weight": 0.02, "mask_l1_weight": 0.01},
]
...
class DomainStratifiedGatedNet(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers, dropout, domain_hidden, treat_indices, physio_indices):
        ...
        # [CLS] token for survival prediction
    ...
    def forward(self, x, grl_coeff=None):
        ...
        tokens = torch.cat((cls_tokens, tokens), dim=1) # Shape: (B, 1+N, d_model)
        
        encoded = self.transformer(tokens)
        
        # 2. Main Survival Prediction (Uses [CLS] token)
...
def build_optimizer(model, phase):
    ...
    if phase == "full_finetune":
        groups = [
            ...
            {"params": [model.feature_weight, model.feature_bias, model.feature_pos, model.cls_token, model.feature_mask], "lr": CONFIG["full_lr"]},
    ...
    for phase, max_epochs, patience in [
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for x_t, e_t, t_t in target_loader:
                ...
                # Add L1 penalty for the mask (average of source and target passes)
