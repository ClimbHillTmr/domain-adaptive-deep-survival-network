"""
v11.2 clean validation + strictly pre-60-min dynamic statistics from upstream data.
...
2. Clean target split: adapt_train / adapt_val / test.
3. Early stopping based on adapt_val only.
"""
...
CONFIG = {
    "source_data": "data_preprocessing/updated_dataset_shenyi.csv",
    "target_data": "data_preprocessing/updated_dataset_fuding.csv",
    "event_col": "透中低血压_计算",
    "duration_col": "et_min",
    "batch_size": 512,
    ...
    "seed": 42,
    "results_path": "runs/v11_2_pre60_da_results.json",
}
    ...
    if "透中数据记录时间节点" in row:
        ...
        if isinstance(ts, str):
            try:
                ...
                parsed = [p.strip() for p in ts.split(",") if p.strip()]
...
def load_feature_matrices():
    df_source = infer_event_and_time(pd.read_csv(CONFIG["source_data"]))
    df_target = infer_event_and_time(pd.read_csv(CONFIG["target_data"]))

    source_event_col = (
        CONFIG["event_col"] if CONFIG["event_col"] in df_source.columns else "events"
    )
    source_duration_col = (
        CONFIG["duration_col"]
        if CONFIG["duration_col"] in df_source.columns
        else "et_min"
    )
    target_event_col = (
        CONFIG["event_col"] if CONFIG["event_col"] in df_target.columns else "events"
    )
    target_duration_col = (
        CONFIG["duration_col"]
        if CONFIG["duration_col"] in df_target.columns
        else "et_min"
    )

    feat_source = build_features_from_upstream(df_source, CONFIG["obs_minutes"])
    feat_target = build_features_from_upstream(df_target, CONFIG["obs_minutes"])
    ...
    e_source = (
        pd.to_numeric(df_source[source_event_col], errors="coerce")
        .fillna(0)
        .values.astype(np.int64)
    )
    t_source = (
        pd.to_numeric(df_source[source_duration_col], errors="coerce")
        .fillna(0)
        .values.astype(np.float32)
    )
    e_target = (
        pd.to_numeric(df_target[target_event_col], errors="coerce")
        .fillna(0)
        .values.astype(np.int64)
    )
    t_target = (
        pd.to_numeric(df_target[target_duration_col], errors="coerce")
        .fillna(0)
        .values.astype(np.float32)
    )

    return x_source, e_source, t_source, x_target, e_target, t_target, common_cols
...
def main():
    ...
    print("1. Loading upstream pre-60-min feature matrices...")
    (
        x_source,
        e_source,
        t_source,
    ...
    print(f"V11.2 clean DA test       : {da_test:.4f}")
