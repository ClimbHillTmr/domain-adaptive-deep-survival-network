"""
v11_train_layerwise_da.py

Supervised domain adaptation for cross-center IDH time-to-event prediction.

Core upgrades over v10:
1. Layer-wise adaptation instead of head-only fine-tuning.
2. Source replay during target adaptation to reduce forgetting.
3. Event-conditional alignment on source/target batches.
"""
...
CONFIG = {
    ...
    "source_replay_weight": 0.4,
    "conditional_align_weight": 0.1,
    "device": "cuda" if torch.cuda.is_available() else "cpu",
    "seed": 42,
    "results_path": "runs/v11_layerwise_da_results.json",
}
...
def run_head_only_adaptation(model, target_loader, target_test, config):
    for param in model.encoder.parameters():
        param.requires_grad = False
    ...
    for epoch in range(1, config["head_only_epochs"] + 1):
        ...
        if score > best_score:
            ...
            best_state = copy.deepcopy(model.state_dict())
        print(
            f"  HEAD_ONLY Epoch {epoch:02d} | loss={total_loss/len(target_loader):.4f} | target_cindex={score:.4f}"
        )
    ...
    for phase, num_epochs in phase_plan:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            print(
                ...
                f"align_loss={total_align/len(target_loader):.4f} | "
...
def main():
    ...
    x_s, e_s, t_s, scaler = load_relative_features(
    ...
    print("\n3. Head-only supervised adaptation benchmark")
    head_model = SurvivalNet(input_dim, CONFIG["d_model"], CONFIG["dropout"]).to(
        CONFIG["device"]
    )
    head_model.load_state_dict(source_state)
    head_only_best, _ = run_head_only_adaptation(
        head_model, target_adapt_loader, target_test, CONFIG
    )

    print("\n4. V11 layer-wise supervised DA with replay + conditional alignment")
    v11_model = SurvivalNet(input_dim, CONFIG["d_model"], CONFIG["dropout"]).to(
        CONFIG["device"]
    )
    v11_model.load_state_dict(source_state)
    v11_best, _, history = run_layerwise_da(
        v11_model,
        source_loader,
        target_adapt_loader,
        target_test,
    ...
    results = {
        ...
        "head_only_target_cindex": head_only_best,
        "v11_layerwise_da_target_cindex": v11_best,
        "delta_vs_zero_shot": v11_best - zero_shot_best,
        "delta_vs_head_only": v11_best - head_only_best,
        "history": history,
        "config": {
    ...
    print(f"Head-only adaptation      : {head_only_best:.4f}")
    print(f"V11 layer-wise DA         : {v11_best:.4f}")
    print(f"Delta vs zero-shot        : {v11_best - zero_shot_best:+.4f}")
    print(f"Delta vs head-only        : {v11_best - head_only_best:+.4f}")
