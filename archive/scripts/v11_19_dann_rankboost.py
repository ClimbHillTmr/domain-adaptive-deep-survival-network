"""
v11.19 add pairwise ranking auxiliary loss on top of best v11.18 DANN setup.
"""
...
CONFIG.update(
    {
        "results_path": "runs/v11_19_dann_rankboost_results.json",
        "seed": 42,
        "d_model": 64,
        "dropout": 0.15,
        "lr": 8e-4,
...
def pairwise_rank_loss(hazard, event, duration, max_pairs=256):
    hazard = hazard.squeeze(-1)
    event = event > 0
    duration = duration.float()
    idx_event = torch.where(event)[0]
    if idx_event.numel() == 0:
        return hazard.new_tensor(0.0)
    if idx_event.numel() > max_pairs:
        perm = torch.randperm(idx_event.numel(), device=hazard.device)[:max_pairs]
        idx_event = idx_event[perm]
    loss_terms = []
    for i in idx_event:
        mask = duration > duration[i]
        mask[i] = False
        j_idx = torch.where(mask)[0]
        if j_idx.numel() == 0:
            continue
        if j_idx.numel() > max_pairs:
            perm = torch.randperm(j_idx.numel(), device=hazard.device)[:max_pairs]
            j_idx = j_idx[perm]
        margin = hazard[i] - hazard[j_idx]
        loss_terms.append(torch.nn.functional.softplus(-margin).mean())
    if not loss_terms:
        return hazard.new_tensor(0.0)
    return torch.stack(loss_terms).mean()


def cox_rank_loss(hazard, event, duration):
    return base.cox_loss(hazard, event, duration) + CONFIG["rank_weight"] * pairwise_rank_loss(hazard, event, duration)
...
def run_source_pretrain(model, source_loader, x_val, e_val, t_val):
    ...
    for epoch in range(1, CONFIG["pretrain_max_epochs"] + 1):
        ...
        for x_s, e_s, t_s in source_loader:
            ...
            loss = cox_rank_loss(hazard, e_s, t_s)
            loss.backward()
            optimizer.step()
...
def run_head_only(model, target_loader, x_val, e_val, t_val):
    ...
    for epoch in range(1, CONFIG["head_only_max_epochs"] + 1):
        ...
        for x_t, e_t, t_t in target_loader:
            ...
            loss = cox_rank_loss(hazard, e_t, t_t)
            loss.backward()
            optimizer.step()
        val_cindex = evaluate_cindex(model, x_val, e_val, t_val)
        if val_cindex > best_val:
...
def run_dann_rank(model, source_loader, target_loader, x_val, e_val, t_val):
    phase_results = []
    for phase, max_epochs, patience in [
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for x_t, e_t, t_t in target_loader:
                ...
                target_loss = cox_rank_loss(hazard_t, e_t, t_t)
                source_loss = cox_rank_loss(hazard_s, e_s, t_s)
...
def main():
    ...
    phase_results = run_dann_rank(da_model, source_loader, target_loader, x_val, e_val, t_val)
