"""
V13.0: IPCW + KAN + Domain-Stratified Gated Survival Network
Upgrades:
1. IPCW (Inverse Probability of Censoring Weighting) to correct censoring shift.
2. Weighted Cox Loss formulation.
3. KAN (Kolmogorov-Arnold Network) Tokenizer option.
4. Preserves V12.1 CLS Token + Gated DANN.
"""
...
CONFIG.update(
    {
        "results_path": "runs/v13_0_ipcw_kan_ds_gsn_results.json",
        "seed": 42,
...
LOCAL_SWEEPS = [
    {"name": "linear_ipcw", "tokenizer_type": "linear", "kan_basis_dim": 0, "adv_weight": 0.05, "mask_l1_weight": 0.05},
    {"name": "kan_ipcw_b8", "tokenizer_type": "kan", "kan_basis_dim": 8, "adv_weight": 0.05, "mask_l1_weight": 0.05},
    {"name": "kan_ipcw_b16", "tokenizer_type": "kan", "kan_basis_dim": 16, "adv_weight": 0.05, "mask_l1_weight": 0.05},
]
...
def compute_ipcw_weights(t_fit, e_fit, t_eval=None):
    """
    Fits KM curve on censoring events (1 - e) to estimate G(t) = P(C > t).
    Returns W_i = 1 / G(T_i) for evaluation times, clipped at 95th percentile.
    """
...
def weighted_cox_loss(hazard, e, t, w):
    """
    IPCW weighted Cox Partial Likelihood.
    L = - sum_i e_i * w_i * [ h_i - log(sum_{j: t_j >= t_i} w_j * exp(h_j)) ]
    """
    ...
    for phase, max_epochs, patience in [
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for x_t, e_t, t_t, w_t in target_loader:
                ...
                target_loss = weighted_cox_loss(hazard_t, e_t, t_t, w_t)
                source_loss = weighted_cox_loss(hazard_s, e_s, t_s, w_s)
...
def main():
    ...
    # Compute IPCW Weights for Source
    w_source = compute_ipcw_weights(t_source, e_source)
    print(f"Source IPCW Weights - Mean: {w_source.mean():.4f}, Max: {w_source.max():.4f}")
    ...
    # Compute IPCW Weights for Target Adapt (Fit only on target train to prevent test leakage)
    w_train = compute_ipcw_weights(t_train, e_train)
    print(f"Target Train IPCW Weights - Mean: {w_train.mean():.4f}, Max: {w_train.max():.4f}")
