...
class UnifiedModelEvaluator:
    ...
    def _create_confusion_matrix(self, y_true, y_pred, model_name):
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                   xticklabels=['正常', '低血压'], yticklabels=['正常', '低血压'])
    ...
    def _create_roc_curve(self, y_true, y_pred_proba, model_name):
        ...
        if self.save_dir:
            plt.savefig(self.save_dir / f'{model_name}_roc_curve.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def _create_regression_plots(self, y_true, y_pred, model_name):
        ...
        if self.save_dir:
            plt.savefig(self.save_dir / f'{model_name}_regression_plots.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def evaluate_model_simple(model, X_test, y_test, model_name="模型", 
                         task_type='classification', save_dir='./evaluation_results',
                         create_visualizations=True, save_results=True):
    """
    快速简化评估模型的便捷函数
