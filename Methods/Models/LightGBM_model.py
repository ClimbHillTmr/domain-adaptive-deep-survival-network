...
def LightGBM_model(
    ...
    if optimization_method == 'bayesian' and BAYESIAN_AVAILABLE:
        ...
        if n_samples >= 150000:  # 大规模数据集（15万+）
            # 针对大规模透析数据的精简参数空间
            param_space = {
                # 核心性能参数
                'n_estimators': [300, 500, 800, 1000, 1200],
                'max_depth': [5, 7, 9, 11],  # 限制深度保证可解释性
                'learning_rate': [0.01, 0.03, 0.05, 0.08],  # 大数据集用较小学习率
                'num_leaves': [31, 63, 127, 255],  # 适中的叶子数
                
                # 正则化参数（防止过拟合）
                'reg_lambda': [0.1, 0.5, 1.0, 5.0],
                'reg_alpha': [0, 0.1, 0.5, 1.0],
                'min_child_samples': [50, 100, 150, 200],  # 大数据集增加最小样本数
                
                # 采样参数（提高训练效率）
                'subsample': [0.7, 0.8, 0.9],
                'colsample_bytree': [0.7, 0.8, 0.9],
                'subsample_freq': [1],  # 透析数据保持连续性
